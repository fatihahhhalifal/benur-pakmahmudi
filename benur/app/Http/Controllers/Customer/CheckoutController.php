...
class CheckoutController extends Controller
{
    ...
    public function processPayment(Request $request): RedirectResponse
    {
        ...
        if ($cartItems->isEmpty()) {
            return redirect()->route('customer.keranjang')->with('error', 'Gagal memproses, keranjang kosong.');
        }

        return DB::transaction(function () use ($request, $userId, $cartItems) {
            $pathBukti = $request->file('bukti_transfer')->store('bukti_dp', 'public');
            ...
            return redirect()->route('customer.pesanan.index')->with('success', 'Berhasil! Preorder dan bukti DP Anda telah terkirim. Admin AQUAFARM akan segera memvalidasi pesanan Anda.');
        });
    }
}