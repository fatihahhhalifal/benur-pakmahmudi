...
class KatalogController extends Controller
{
    ...
    public function uploadPelunasan(Request $request, int $id): RedirectResponse
    {
        $request->validate([
            'bukti_transfer_pelunasan' => 'required|image|mimes:jpeg,png,jpg|max:3072'
        ]);
        ...
        if (!$pesanan || $pesanan->status !== 'menunggu_pelunasan') {
            return redirect()->back()->with('error', 'Status pesanan tidak valid untuk memproses pelunasan.');
        }

        $pathBukti = $request->file('bukti_transfer_pelunasan')->store('bukti_pelunasan', 'public');

        DB::table('pesanan')->where('id', $id)->update([
            'bukti_transfer_pelunasan' => $pathBukti,
            'updated_at' => now()
        ]);

        return redirect()->back()->with('success', 'Bukti transfer pelunasan berhasil diunggah. Menunggu verifikasi dari Admin.');
    }
}
