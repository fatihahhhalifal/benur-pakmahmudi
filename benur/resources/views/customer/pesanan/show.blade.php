<x-customer-layout>
<div x-data="{ isLightboxOpen: false, activeImage: '', fileNamePelunasan: null }" class="font-sans text-slate-800 min-h-full bg-[#F8FAFC]">
        ...
        @php
            $allSteps = [
                [
                    ...
                    'desc'  => 'Pesanan diterima. Bukti transfer DP Anda sedang diperiksa oleh Admin.',
                ],
                [
                    'key'   => 'proses',
                    'label' => 'DP Diverifikasi / Siap Muat',
                    'icon'  => 'fa-truck-ramp-box',
                    'desc'  => 'DP dikonfirmasi. Benur sedang disiapkan dan dimuat ke kendaraan Anda.',
                ],
                [
            ...
            // Apakah customer sudah upload bukti pelunasan tapi admin belum verifikasi
                ...
                <div class="mt-4 {{ $activeBannerColor['bg'] }} {{ $activeBannerColor['border'] }} border rounded-xl p-3 flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[10px] font-black {{ $activeBannerColor['text'] }}">
                            {{-- Override label jika sudah upload pelunasan tapi belum dikonfirmasi --}}
                        ...
                        <p class="text-[9px] text-slate-500 font-medium mt-0.5 leading-snug">
                            @if($pesanan->status === 'menunggu_pelunasan' && $sudahUploadPelunasan)
                                Bukti transfer Anda sudah diterima. Admin sedang memverifikasi pembayaran. Surat jalan akan diterbitkan setelah konfirmasi selesai.
                            @else
                                {{ $activeStep['desc'] }}
                            @endif
                        </p>
            ...
            <div class="lg:col-span-2 space-y-5">
                            ...
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-[9px] {{ $pesanan->status=='pending' ? 'text-amber-500' : 'text-emerald-500' }} font-bold mt-0.5">
                                        {{ $pesanan->status=='pending' ? 'Sedang diverifikasi Admin.' : 'Telah terverifikasi.' }}
                                    </p>
                                </div>
                ...
                @if($pesanan->status == 'menunggu_pelunasan')
                    @if($sudahUploadPelunasan)
                        {{-- Bukti sudah diupload, tinggal tunggu admin --}}
                        <div class="bg-blue-50 border border-blue-200 rounded-2xl p-5 flex items-start gap-4">
                            ...
                            <div>
                                <p class="text-xs font-black text-blue-700 mb-1">Bukti pelunasan sudah diterima</p>
                                <p class="text-[10px] text-slate-500 font-medium leading-relaxed">
                                    Terima kasih! Bukti transfer Anda sudah kami terima dan sedang dalam proses verifikasi oleh Admin.
                    ...
                    @else
                        ...
                        <form action="{{ route('customer.pesanan.pelunasan', $pesanan->id) }}" method="POST"
                              enctype="multipart/form-data"
                              @submit="if(!fileNamePelunasan){ $event.preventDefault(); alert('Pilih foto bukti transfer pelunasan terlebih dahulu!'); }"
                              class="bg-white rounded-2xl border border-rose-200 shadow-sm p-5">
                            @csrf
                            <h3 class="text-xs font-black text-rose-600 uppercase tracking-wider mb-1 flex items-center gap-1.5">
                                <i class="fa-solid fa-cloud-arrow-up"></i> Upload Bukti Pelunasan
                                <span class="ml-auto bg-rose-100 text-rose-700 text-[8px] font-black px-2 py-0.5 rounded-full">Aksi Diperlukan</span>
                            </h3>
                            <p class="text-[10px] text-slate-500 mb-4 leading-relaxed">
                                Transfer sisa tagihan sebesar <strong class="text-rose-600">Rp{{ number_format($sisaPelunasan,0,',','.') }}</strong>
                                lalu upload bukti transfernya di sini.
                            </p>
                            <label class="block cursor-pointer mb-4">
                                <input type="file" name="bukti_transfer_pelunasan" accept="image/*" class="hidden"
                                       @change="fileNamePelunasan = $event.target.files[0].name" required>
                                <div class="border border-dashed border-rose-300 bg-rose-50/50 rounded-xl py-6 px-4 text-center hover:bg-rose-50 transition-colors">
                                    <div class="w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2 shadow-sm text-white"
                                         :class="fileNamePelunasan ? 'bg-emerald-500 shadow-emerald-500/30' : 'bg-rose-500 shadow-rose-500/30'">
                                        <i class="fa-solid" :class="fileNamePelunasan ? 'fa-check' : 'fa-camera'"></i>
                                    </div>
                                    <p class="text-xs font-black" :class="fileNamePelunasan ? 'text-emerald-600' : 'text-rose-600'"
                                       x-text="fileNamePelunasan || 'Klik untuk pilih foto resi pelunasan'"></p>
                                </div>
                            </label>
                            <button type="submit"
                                class="w-full bg-rose-600 hover:bg-rose-700 text-white font-black text-xs py-3.5 rounded-xl shadow-lg shadow-rose-500/20 transition-all active:scale-95 flex items-center justify-center gap-2">
                                <i class="fa-solid fa-paper-plane text-xs"></i> Kirim Bukti Pelunasan
                            </button>
                        </form>
                    @endif
                @endif
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
                        <p class="text-[9px] font-black text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                            <i class="fa-solid fa-file-invoice text-blue-500"></i> Bukti Transfer DP
                        </p>
                        <div @click="activeImage='{{ $pesanan->bukti_transfer_dp ? asset('storage/'.$pesanan->bukti_transfer_dp) : '' }}'; if(activeImage) isLightboxOpen=true"
                             class="w-full aspect-[3/4] bg-slate-50 rounded-xl overflow-hidden border border-slate-200 relative group cursor-zoom-in">
                            @if($pesanan->bukti_transfer_dp)
                                <img src="{{ asset('storage/'.$pesanan->bukti_transfer_dp) }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                    ...
                    @if(!empty($pesanan->bukti_transfer_pelunasan))
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
                        <p class="text-[9px] font-black text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                            <i class="fa-solid fa-file-invoice-dollar text-emerald-500"></i> Bukti Transfer Lunas
                        </p>
                        <div @click="activeImage='{{ asset('storage/'.$pesanan->bukti_transfer_pelunasan) }}'; isLightboxOpen=true"
                             class="w-full aspect-[3/4] bg-slate-50 rounded-xl overflow-hidden border border-slate-200 relative group cursor-zoom-in">
                            <img src="{{ asset('storage/'.$pesanan->bukti_transfer_pelunasan) }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <i class="fa-solid fa-magnifying-glass-plus text-white text-2xl"></i>
                            </div>
                        </div>
