        ...
        foreach ($pesanan as $p) {
            ...
            if (!isset($grouped[$p->id])) {
                $grouped[$p->id] = [
                    ...
                    'bukti_dp' => !empty($p->bukti_transfer_dp) ? asset('storage/' . $p->bukti_transfer_dp) : null,
                    'bukti_lunas' => !empty($p->bukti_transfer_pelunasan)
                        ? asset('storage/' . $p->bukti_transfer_pelunasan)
                        : null,
                    'status' => $p->status,
                    ...
                    'url_verifikasi' => url('/admin/pesanan/' . $p->id . '/verifikasi-dp'),
                    'url_kalkulasi' => url('/admin/pesanan/' . $p->id . '/kalkulasi-final'),
    ...
    <div class="max-w-7xl mx-auto font-sans text-slate-800 antialiased" x-data="adminOrderManager({{ json_encode($ordersData) }})">
        ...
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                ...
                @if (Auth::user()->role === 'admin')
                    <p class="text-[12px] text-slate-500 font-bold">Pusat kendali verifikasi uang muka dan eksekusi timbang muat lapangan.</p>
                @else
        ...
        @if (Auth::user()->role === 'admin')
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-gradient-to-br from-amber-500 to-orange-600 backdrop-blur-xl border border-white/20 rounded-3xl p-5 shadow-[0_8px_30px_rgba(245,158,11,0.25)] relative overflow-hidden flex items-center gap-4 text-white">
                    ...
                    <div class="relative z-10">
                        <p class="text-[10px] font-black uppercase tracking-widest mb-1 text-amber-100">Validasi DP</p>
                    ...
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200 text-[11px] font-black text-slate-500 capitalize whitespace-nowrap">
                            ...
                            @if (Auth::user()->role === 'admin')
                                ...
                                <th class="p-4 min-w-[150px]">Bukti Transaksi</th>
                            @endif
                            ...
                            <tr class="hover:bg-blue-50/30 transition-colors">
                                ...
                                @if (Auth::user()->role === 'admin')
                                    ...
                                    <td class="p-4 whitespace-nowrap align-top">
                                        <div class="flex flex-col gap-1.5">
                                            <template x-if="p.bukti_dp">
                                                <button type="button"
                                                    @click="showFullscreen(p.bukti_dp, 'Bukti Transfer DP - ' + p.nomor_invoice)"
                                                    class="inline-flex items-center gap-1.5 text-[10px] font-bold bg-amber-50 text-amber-600 hover:bg-amber-100 px-2 py-1.5 rounded-lg border border-amber-200 transition-colors w-max shadow-sm">
                                                    <i class="fa-solid fa-expand"></i> Zoom Bukti DP
                                                </button>
                                            </template>
                                            <template x-if="!p.bukti_dp">
                                                <span class="text-[10px] text-slate-400 italic block mt-1">Belum Upload DP</span>
                                            </template>
                                            <template x-if="p.bukti_lunas">
                                                <button type="button"
                                                    @click="showFullscreen(p.bukti_lunas, 'Bukti Transfer Pelunasan - ' + p.nomor_invoice)"
                                                    class="inline-flex items-center gap-1.5 text-[10px] font-bold bg-emerald-50 text-emerald-600 hover:bg-emerald-100 px-2 py-1.5 rounded-lg border border-emerald-200 transition-colors w-max mt-1 shadow-sm">
                                                    <i class="fa-solid fa-expand"></i> Zoom Pelunasan
                                                </button>
                                            </template>
                                        </div>
                                    </td>
                                ...
                                <td class="p-4 whitespace-nowrap align-top">
                                    <template x-if="p.status == 'pending'">
                                        <span class="bg-amber-50 text-amber-600 border border-amber-200 px-2.5 py-1 rounded-md text-[10px] font-black uppercase shadow-sm">Validasi DP</span>
                                    </template>
                                        ...
                                        @if (Auth::user()->role === 'admin')
                                            <template x-if="p.status == 'pending'">
                                                <div class="flex gap-2">
                                                    <button @click="openDp = true; selectedOrder = p"
                                                        class="bg-amber-500 hover:bg-amber-600 text-white text-[10px] uppercase px-3 py-1.5 rounded-lg font-black transition-all shadow-sm">Verifikasi DP</button>
                                                    <button @click="openBatal = true; selectedOrder = p"
                                                        title="Batalkan Pesanan"
                                                        class="bg-rose-50 hover:bg-rose-600 text-rose-500 hover:text-white text-[12px] w-8 h-8 flex items-center justify-center rounded-lg transition-all border border-rose-200 hover:border-rose-600 shadow-sm">
        ...
        @if (Auth::user()->role === 'admin')
            ...
            <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-sm"
                x-show="openPreview" x-transition.opacity x-cloak>
                <button type="button" @click="openPreview = false"
                    class="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center text-xl transition-colors z-[110]"><i class="fa-solid fa-xmark"></i></button>
                <div class="absolute top-6 left-6 text-white text-sm font-black uppercase tracking-widest" x-text="previewTitle"></div>
                <img :src="previewSrc" class="max-w-full max-h-screen object-contain p-4 select-none cursor-zoom-out"
                    @click.away="openPreview = false" @click="openPreview = false">
                <a :href="previewSrc" download
                    class="absolute bottom-8 right-8 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-2 transition-colors shadow-2xl z-[110]"><i class="fa-solid fa-download"></i> Unduh Asli</a>
            </div>

            {{-- MODAL VALIDASI DP --}}
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm"
                x-show="openDp" x-transition.opacity x-cloak>
                <div class="bg-white rounded-[24px] max-w-sm w-full p-6 shadow-2xl border border-slate-100" @click.away="openDp = false">
                    <div class="flex items-center gap-3 text-amber-500 mb-3">
                        <div class="w-10 h-10 rounded-full bg-amber-50 flex items-center justify-center"><i class="fa-solid fa-shield-check text-xl"></i></div>
                        <h3 class="text-base font-black text-slate-900">Verifikasi Uang Muka</h3>
                    </div>
                    <form :action="selectedOrder.url_verifikasi" method="POST">
                        @csrf
                        <input type="hidden" name="nominal_dp" :value="selectedOrder.kewajiban_dp">
                        <p class="text-[12px] text-slate-500 mb-4 font-medium leading-relaxed">
                            Validasi bukti transfer DP untuk invoice <span class="font-black text-blue-600" x-text="selectedOrder.nomor_invoice"></span>.
                        </p>
                        <div class="bg-slate-50 border border-slate-200 rounded-xl p-3 mb-3 text-[11px] space-y-2 max-h-40 overflow-y-auto">
                            <p class="font-black text-slate-600 mb-1">Detail Pesanan:</p>
                            <template x-for="k in (selectedOrder.kolams || [])" :key="k.detail_id">
                                <div class="border-b border-slate-200 last:border-0 pb-1.5 last:pb-0 space-y-0.5">
                                    <p class="font-black text-slate-700" x-text="k.nama_kolam"></p>
                                    <div class="flex justify-between"><span class="text-slate-500">Sak:</span><span class="font-black" x-text="k.jumlah_sak + ' Sak'"></span></div>
                                    <div class="flex justify-between"><span class="text-slate-500">Kantong:</span><span class="font-black" x-text="formatRupiah(k.total_kantong_draf) + ' Ktg'"></span></div>
                                    <div class="flex justify-between"><span class="text-slate-500">Est. Ekor:</span><span class="font-black text-blue-600" x-text="formatRupiah(k.total_ekor_draf) + ' Ekor'"></span></div>
                                </div>
                            </template>
                        </div>
                        <div class="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl mb-5">
                            <span class="text-[10px] font-black text-emerald-600 uppercase tracking-wider block mb-1">Nilai Wajib (DP 20%)</span>
                            <span class="text-xl font-black text-emerald-700" x-text="'Rp ' + formatRupiah(selectedOrder.kewajiban_dp)"></span>
                        </div>
                        <div class="flex gap-3">
                            <button type="button" @click="openDp = false" class="flex-1 bg-slate-100 py-3 text-[12px] rounded-xl font-black text-slate-600 hover:bg-slate-200 transition-colors">Batal</button>
                            <button type="submit" class="flex-1 bg-amber-500 py-3 text-[12px] rounded-xl font-black text-white shadow-lg shadow-amber-500/30 hover:bg-amber-600 transition-transform active:scale-95">Konfirmasi Sah</button>
                        </div>
                    </form>
                </div>
            </div>
